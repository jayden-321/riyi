...
当前客户端构建号为 **37**，包含可编辑的训练／休息日日历与恢复活动、组团打卡、Apple 健康睡眠与体征、AI 教练、饮食文字与拍照粗估和新图标。饮食记录默认使用当前时刻；文字或照片粗估要求本人在 App 中同意 AI 使用数据，返回的范围需核对，份量不清时可先保存为“热量待估算”。教练请求断线后按请求号恢复同一次结果，避免盲目重复生成；云端登录过期时会提示重新登录，“我的”也有固定入口；只能重新登录同一账号，原本机记录会保留。清空教练聊天后，定时训练／睡眠分析仍可从「分析记录」单独查看。拍包装识别会自动填切片数量单位；多口味能量以同口径最高每百克标签值供核对。核对保存后会自动生成分享码，新商品默认可由其他用户按名称或品牌搜索；分享仅包含商品资料，不包含饮食记录，仍可关闭搜索或撤销分享。导入商品可留空搜索全部公开可搜索商品并分页加载；AI 可把每份营养表按可见份量换算为每百克。默认服务器已部署对应 API；此公开仓库仅含客户端。
