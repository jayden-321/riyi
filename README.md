...
当前客户端构建号为 **55**。训练页按日历日期查看计划与历史，同一天可按顺序安排多个独立项目，例如力量后游泳；新增项目不会覆盖原安排。已完成实际训练但还没有日历计划的日期，也能由用户补排计划；能明确匹配的实际训练合并显示在该计划下。手动编辑周期和 AI 健康安排均支持同日多项目。计划编辑先选运动大类；力量动作从动作库选，组合直接关联已选动作，计划容量按正式组重量与次数自动计算；其他运动按时间、距离和大卡目标安排。手表可左右切换当天项目，训练可在双端暂停、继续或停止保存，结束后可重新开始。日益 App 的实际训练可在本人授权后写回 Apple 健康。默认服务器为 **48** 号 API；此公开仓库仅含客户端。

HIIT 已单列为运动大类；教练设置可保存每次动作数与每动作组数。喝水通知正文会打开饮食里的饮水记录，不自动填写饮水量；通知的快捷操作可直接记录 250/500 ml。教练页面隐藏框架来源和冗长的分析依据说明。
...
> 请先 `git pull --ff-only origin main` 更新日益客户端源码，并保护我本机未提交的修改。阅读 README 与 `ios/project.yml`，用我自己的 Apple 账号 Personal Team 给 iPhone 和已配对的 Apple Watch 安装构建 **55** 的「日益」：两端必须使用同一个 Team、同一张 Apple Development 证书，各自的描述文件覆盖对应 Bundle ID 与设备。在本地填写独有的 `RIYI_IOS_BUNDLE_ID` 和共用 `DEVELOPMENT_TEAM`，重新运行 `xcodegen generate`，再用 Xcode 构建、安装并启动两端。手机欢迎页选择「已有账号？登录」，使用我自己的云端账号和默认服务器 `https://health.qyos.top`；**不要选择本地演示**，也不要使用仓库作者账号。请让我在设备上亲自输入密码并授予 Apple 健康、云端上传、AI 分析和训练写入权限。登录后核对云端数据、同日力量加游泳是否各显示一次且分别可开始、已完成训练的日期能否补排计划、手动／AI 周期入口、手表状态及饮食。只凭 BUILD SUCCEEDED 或已安装不能算完成；分别报告两端签名、安装、启动、同步与实际限制。不要部署或修改服务器，不索取 Apple 密码，不把个人签名配置、密钥或健康记录推回 GitHub。
...
浏览器版训练日历：`https://health.qyos.top/app/`。使用本人日益账号登录，可在电脑安排或编辑每天的训练；完成记录仍由手机和手表产生。
