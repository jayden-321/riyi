...
当前客户端构建号为 **57**。训练页按日历日期查看计划与历史，同一天可按顺序安排多个独立项目，例如力量后游泳；新增项目不会覆盖原安排。已完成实际训练但还没有日历计划的日期，也能由用户补排计划；能明确匹配的实际训练合并显示在该计划下。手动编辑周期和 AI 健康安排均支持同日多项目。计划编辑先选运动大类；力量动作从动作库选，组合直接关联已选动作，计划容量按正式组重量与次数自动计算；其他运动按时间、距离和大卡目标安排。手表可左右切换当天项目，训练可在双端暂停、继续或停止保存，结束后可重新开始。日益 App 的实际训练可在本人授权后写回 Apple 健康。默认服务器为 **50b** 号 API；此公开仓库仅含客户端。

HIIT 已单列为运动大类；教练设置可保存每次动作数与每动作组数。饮食「拍照」先进入外食记录页，再选择拍照或从相册选。公开可搜索商品支持举报、屏蔽分享来源与解除屏蔽；举报商品待人工复核时暂不显示。

喝水通知正文会打开饮食里的饮水记录，不自动填写饮水量；通知的快捷操作可直接记录 250/500 ml。教练对话和饮食问答并发显示 A（日益当前方案）与 B（完整 Skill 参考）；只有 A 的训练或饮食草稿可以安排到日历，B 仅供比较。
