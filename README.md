...
当前客户端构建号为 **52**。训练页按日历日期查看计划与历史，同一天可按顺序安排多个独立项目，例如力量后游泳；新增项目不会覆盖原安排。已完成实际训练但还没有日历计划的日期，也能由用户补排计划；能明确匹配的实际训练合并显示在该计划下。手动编辑周期和 AI 健康安排均支持同日多项目。计划编辑先选运动大类；力量动作从动作库选，组合直接关联已选动作，计划容量按正式组重量与次数自动计算；其他运动按时间、距离和大卡目标安排。手表可左右切换当天项目，训练可在双端暂停、继续或停止保存，结束后可重新开始。日益 App 的实际训练可在本人授权后写回 Apple 健康。默认服务器为 **45** 号 API；此公开仓库仅含客户端。

HIIT 已单列为运动大类；教练设置可保存每次动作数与每动作组数。已练记录有独立训练总结与分享预览页，有采样时显示心率；Apple 健康若确实记录了对应训练的活动能量才显示千卡。分享图片在本机生成，缺失的重量、心率或热量不会补算。手表完成全部组后显示“完成训练”，休息结束的声音提醒需在手表允许日益通知。
