...
正式客户端固定连接 `https://health.gzqy.xyz`，登录页不提供服务器地址切换或新的本地体验入口。已有账号选择「已有账号？登录」；没有账号可注册；忘记密码入口待 iCloud SMTP 凭据配置后通过邮件发送验证码。旧 `health.qyos.top` 由服务器兼容转发，更新时保留原账号的本机记录空间。Apple 健康读取、健康上传和 AI 分析分别由本人在 App 中授权。请勿使用仓库作者的账号或密钥。客户端仓库不需要在你的电脑上部署数据库或后端。

当前客户端构建号为 **59**。训练页按日历日期查看计划与历史，同一天可按顺序安排多个独立项目，例如力量后游泳；新增项目不会覆盖原安排。已完成实际训练但还没有日历计划的日期，也能由用户补排计划；能明确匹配的实际训练合并显示在该计划下。手动编辑周期和 AI 健康安排均支持同日多项目。计划编辑先选运动大类；力量动作从动作库选，组合直接关联已选动作，计划容量按正式组重量与次数自动计算；其他运动按时间、距离和大卡目标安排。手表可左右切换当天项目，训练可在双端暂停、继续或停止保存，结束后可重新开始。日益 App 的实际训练可在本人授权后写回 Apple 健康。默认服务器为 **54** 号 API；此公开仓库仅含客户端。
...
找回密码邮件使用 iCloud SMTP；服务器需配置 Apple App 专用密码后才能发信。代码与接口已部署，但在凭据完成配置和实际收件验收前，不能将“邮件找回已可用”视为完成。
